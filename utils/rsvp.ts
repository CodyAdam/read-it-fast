 ...
 * @param {string} newLineChar - Character used to represent new lines (default: "›")
...
export function getWordDelay(
  ...
  if (pauseOnPunctuation) {
    ...
    if (new RegExp(`[.!?;:${newLineChar}]$`).test(word)) {
 ...
 */
export function shouldPauseAtWord(
  wordIndex: number,
...
export function extractWordFrame(
  ...
  centerIdx: number,
  frameSize: number
): { subset: string[]; centerOffset: number } {
